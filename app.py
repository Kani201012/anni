...
with st.sidebar:
    ...
    with st.expander("🎨 Visual DNA", expanded=True):
        theme_mode = st.selectbox("Base Theme", ["Clean Corporate (Light)", "Midnight SaaS (Dark)", "Glassmorphism (Blur)"])
        c1, c2 = st.columns(2)
...
def gen_inventory_js():
    ...
    function parseCSV(str) {{
        const arr = []; let quote = false; let col = 0, c = 0;
        for (; c < str.length; c++) {{
            let cc = str[c], nc = str[c+1]; arr[col] = arr[col] || '';
    ...
    <footer><div class="container">
        <div class="footer-grid">
            <div>
                ...
                <p style="opacity:0.6; font-size:0.9rem;">{biz_addr}</p>
                <p style="opacity:0.6; font-size:0.9rem; margin-top:1rem;">{biz_email}</p>
            </div>
            <div>
...
with c1:
    st.markdown("Previewing **Home Page** structure...")
